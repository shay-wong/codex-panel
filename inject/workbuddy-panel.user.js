(() => {
  ...
  const ENTRY_ID = "workbuddy-panel-entry";
  const PANEL_ID = "workbuddy-panel-root";
  const FRAME_ID = "workbuddy-panel-frame";
  const PANEL_URL = "http://127.0.0.1:47823/?project=local&host=workbuddy";

  window.__workbuddyPanel?.destroy?.();
  ...
  window.__workbuddyPanel = {
    ...
    destroy() {
      ...
      entry.remove();
      panel.remove();
      delete window.__workbuddyPanel;
    },
  };
})();
