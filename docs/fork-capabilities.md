...
When Jira and every linked execution issue are done, Panel archives the Jira planning conversation and the Panel-managed execution conversations associated through simple start, automatic claiming, or rework. Archived conversations leave the active list but retain their local thread, run, event, and Codex thread records for direct historical reads. A currently running turn remains visible until it settles. Reopened work creates a new rework or planning conversation instead of resuming an archived one.
