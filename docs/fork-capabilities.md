...
Panel stores the path to each Jira CLI configuration, not Jira credentials. The provider API does not query Jira; after a successful settings save, the launcher-managed Panel embedded inside Codex separately reconciles the provider's Scheduled Task.

## Jira synchronization Scheduled Tasks

For each enabled Jira provider, Panel manages at most one clearly owned projectless Codex Scheduled Task. This does not restrict user-created Scheduled Tasks, and an identical display name alone never grants Panel ownership. The managed task runs daily at 09:00 Asia/Shanghai, and Codex performs its normal single catch-up when the device becomes available after a missed run. “Run now” can be used repeatedly, but Panel rejects another run while the same provider already has one in progress.

The generated task uses only that provider's Jira CLI config path and JQL. Editable alias, config path, and JQL values are encoded as CLI data instead of being inserted into the Agent's natural-language instructions. Jira issues and linked requirement documents are treated as external data, never as instructions. The task reads the complete matching Jira result set, current Panel projects and issues, and only explicitly linked requirement evidence available through authenticated read-only tools. It emits a versioned JSON plan with provider and run identity, snapshots, evidence, proposals, ambiguities, and failures. The task does not create, modify, move, archive, or delete Panel issues and does not write back to Jira. Runs with no proposals, ambiguities, or failures archive their own Codex task; runs needing attention remain visible with a short summary.

Panel compares the native task with its canonical template and shows whether it is normal, externally modified, missing, or conflicting. Saving changes to an existing provider, including enabling or disabling it, never rewrites the native task; “Restore standard task” explicitly applies the current provider definition and status. A still-bound task must carry the provider's exact machine marker; when the stored ID is stale or foreign, Panel may rebind only one uniquely marked task from the complete list. Same-name tasks without that marker remain user-owned and untouched. Multiple exact markers produce a conflict and Panel performs no creation, update, run, or deletion until the user resolves it in Codex Scheduled Tasks. A newly created task is re-listed and ownership-confirmed before Panel pauses it or reports it as normal. Restoring a missing task for a disabled provider recreates it in the paused state. If pausing that confirmed task fails, Panel keeps its ID and reports drift so restoration can be retried without losing the active task. Deleting a provider proceeds only after its uniquely owned task is confirmed paused or missing, so deletion is unavailable in a normal browser that cannot inspect native tasks. Scheduled Task inspection, restoration, and manual runs are available only from the launcher-managed Panel embedded inside Codex; a normal browser can still edit provider settings and points users to the Jira settings in that embedded Panel, not the standalone manager window.
