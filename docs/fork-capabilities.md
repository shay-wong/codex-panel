...
The queue persists its source, state, selected conversation, attempts, next retry time, and last error. Dispatch prefers manual and resumed work; Jira-authorized and scanned work then share the issue-priority and board-order queue. Each issue reuses its existing execution conversation when available, otherwise Panel creates one conversation and starts it with `implement`. A successful run must leave the issue in review or done; a run that needs input leaves it blocked. A user comment or completed user turn resumes blocked work. Transient connection or startup failures retry after 30 seconds and 2 minutes; other failures stop the claim, block the issue, and add an agent comment. A service restart records the interrupted attempt, returns the issue to waiting, and resumes the same queued claim and conversation.

This release intentionally dispatches only one automatic execution at a time. Per-project parallelism and worktree isolation are separate follow-up work. Panel asks only its own previously saved legacy automation ID to pause during migration; it never scans or changes user-created Codex Scheduled Tasks.
