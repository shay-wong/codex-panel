...
For a complex requirement, choose **Plan with AI** to open a separate conversation with the Jira description, requirement links, and selected repositories. Newly created planning conversations default to **Approve selected actions** (`workspace-write`); an existing planning conversation keeps its saved permission. Planning remains available before a repository is selected; Panel explicitly allows that managed non-Git workspace, and a Codex startup failure shows its actual diagnostic. The conversation runs `grill-me`, `to-spec`, and `to-tickets`; the resulting Spec is stored on the Jira requirement rather than becoming a claimable issue. Publishing requires user approval and a selected linked repository for every ticket. It creates linked backlog issues, preserves declared blocking relationships across repositories, and does not authorize execution. Jira description lines whose numbered items begin with `#` render as an ordered list without changing ordinary Panel Issue Markdown.
