...
The queue persists its source, state, selected conversation, attempts, next retry time, and last error. Dispatch prefers manual and resumed work; Jira-authorized and scanned work then share the issue-priority and board-order queue. Capacity keeps an issue waiting in `todo`. Formal execution opens the selected repository in Codex, selects the native **New local worktree** run location, and submits the prepared `manage-panel` and `implement` prompt. Codex therefore owns worktree creation and runs the project's configured environment setup; Panel stores the actual native thread binding, worktree path, and branch only after Codex returns them. Jira-linked execution uses the Jira external key in its task title, prompt, and branch naming instruction instead of the Panel issue identifier. A successful execution must leave the issue in review or done; execution that needs input leaves it blocked. A user comment or completed user turn resumes blocked work. Transient connection or startup failures retry after 30 seconds and 2 minutes; other failures stop the claim, block the issue, and add an agent comment.
