...
未关联仓库时也可“识别仓库并开始”：Codex 只读识别仓库及各自范围，明确后进入现有执行队列；有歧义时在详情内补充回答，这条执行路径不要求 Spec。单个已关联仓库直接执行，多个已关联仓库保留选择并分别确定范围。自动识别本身不会创建规划会话，但开始执行后仍可点击“AI 规划”，创建正式规划会话并保存 Spec，已有执行工作会继续保留。
...
普通任务（包括 backlog 主任务）也可以使用 **AI 规划**：默认预填原生 Plan 草稿，手动发送后只绑定会话，不进入执行。详情页的 **规划 Spec** 支持编写、编辑与持久化查看；Agent 使用 `panelctl issue planning get/save`，保存前读取 `plan.version`，过期版本会拒绝覆盖。Spec 保留在主任务，不替换描述。确认拆分后，可用 `issue create --status backlog` 和 `issue relation` 创建同一项目内的多个子任务及前置依赖，已有子任务继续复用，创建不会启动执行。之后明确授权实现时，可以在原会话继续，并读取已保存的 Spec。只有 Jira 需求才增加关联仓库、Jira 状态同步和 `jira planning publish` 发布处理。
...
Panel 已加入 Codex 原生导航历史：返回回到打开面板前的页面，前进重新打开面板；从面板进入原生目的地后，也能返回面板。显示状态跟随实际路由，不再匹配按钮或命令标题；切换主题等不导航的操作保持面板打开。

面板加载失败时可点击“重新加载”在当前页面重试；若提示启动器未连接，先在 Codex Panel 中启动或重启服务。详见[面板导航与恢复](docs/fork-capabilities.md#switch-between-panel-and-native-codex-destinations)。

每个项目仪表盘每天生成一次 Codex 项目总结。生成失败后，Panel 会依次等待 5、15、60 分钟重试，随后停止自动尝试；仪表盘仍保留手动重试入口，任意一次生成成功都会重置失败次数。

Jira 活动中的 Issue 关联和解除记录都能打开对应的 Panel Issue，即使目标仓库没有加载在当前 Jira 视图中，历史解除记录也仍然可跳转。

面板支持概览、列表、甘特图和归档 Issue 工作流。横向列表沿用议题看板的分栏与卡片层级，竖向列表保持紧凑；同步自 Jira 的 Issue 会优先显示外部 Key。Issue 可以设置开始与截止日期，也可以从详情页移动到其他项目并保留关联对话。本地 Jira 连接会把分配给当前 Jira 登录用户的未完成任务同步到固定 Jira 项目。每个 Jira 需求可以选择一个或多个仓库项目，并关联这些仓库中已有的执行 Issue；可搜索、可滚动的仓库选择器会列出本机 Codex 的全部仓库，尚未进入 Panel 的仓库只在保存关联时注册为项目。每个执行 Issue 最多属于一个 Jira 需求，Jira 刷新不会覆盖本地工作内容，活动记录中的仓库变更显示项目名称而不是内部 ID。执行 Issue 上的关联 Jira 卡片会返回 Panel 内的需求详情，外部 Jira 入口仍保留在该详情页；关联仓库较多时摘要显示首个仓库和 `+N`，悬停可查看完整列表。已保存仓库关联的待认领 Jira 可以一键切换到进行中，为每个仓库创建一个执行 Issue 和一个正式的原生 Codex 任务，并在所有仓库就绪后再统一把 Issue 释放到待认领；部分失败后重试会继续原操作，不会重复创建。复杂需求则可以打开默认使用“帮我批准”（`workspace-write`）权限的正式原生 Codex 规划任务：未关联仓库时创建无项目任务，关联一个仓库时直接在该项目下创建，关联多个仓库时先选择本次规划使用的项目。Panel 会把生成的规划 prompt 与全局选择的规划 Skill 填入可编辑输入框；没有可用自定义 Skill 时使用 Codex 原生 Plan，由用户检查后手动发送。原生规划确认后，需要退出 Plan 模式再保存 Spec 或发布 tickets。正式规划和执行任务通过 Codex 原生任务路由打开，不会出现在右下角仅用于临时问答的内嵌聊天中；`~/.agents/skills` 下以直接符号链接安装的用户 Skill 仍会作为真实 Skill 引用，而不是普通 `$名称` 文本。如果草稿发送后、Panel 记录绑定前 renderer 重载，待绑定意图仍会保留；再次点击规划会按精确 Jira Key 恢复唯一匹配的 Codex 会话，不会重复创建。生成的 Spec 保存在 Jira 需求上，已确认仓库的 tickets 会发布为带依赖关系的已关联 backlog Issue。即使尚未选择仓库也可以开始规划，发送后 Codex 启动失败时会显示实际诊断。Jira 描述中以 `#` 开始的编号项会按有序列表显示，不会改写普通 Panel Issue 的 Markdown。Jira 内容或仓库变化后必须重新复核才能再次发布，未开始 Issue 在此期间不能进入执行，重新规划只会取消被替代且尚未开始的工作；已开始成果会作为新计划约束继续显示。打开 Jira 项目时最多每分钟刷新一次，也可以手动同步。只有全部 Jira 搜索分页成功后才会应用刷新；Jira 不可用时仍显示缓存任务，切换到其他 Jira 登录账号前必须确认。Jira 顶部和设置中会显示最后尝试、最后成功、未完成与状态未知数量，以及可执行的失败原因。修改已同步 Jira Issue 的标题、描述、优先级、标签、截止日期或状态时，Panel 会直接回写 Jira。连接支持账号密码或 Jira Cloud 邮箱与 API Token 的 Basic Auth，也支持 Jira Data Center 或 Server Personal Access Token 的 Bearer Auth。凭据保存在 Panel 本地数据目录中，云端模式不可用；除非 Jira 位于受信任的内网，否则应使用 HTTPS。
...
仓库项目使用 Panel 自己持久化的执行队列，不再依赖 Codex Scheduled Task。可以在项目自动化菜单开启自动认领，按 5、10、15、30 或 60 分钟扫描未关联 Jira 的待认领 Issue；Jira 关联 Issue 会在其 Jira 需求授权执行后进入队列。手动入口改为“准备执行”，只附加 Manage Panel，读取任务后按需加载工作流配置；默认使用当前目录，也可选择新建 worktree，已有执行上下文会复用。用户在 Codex 中发送后才关联对话并进入处理中。自动认领仍自动发送；关闭自动认领只阻止新的自动入队，不会冻结已有队列，暂停项目则会让全部排队项继续等待。全局默认每个项目同时执行 3 个 Issue，可在 1-8 内调整；每个项目可以跟随默认值或单独覆盖为 1-8，不设置跨项目总上限。新的自动执行会选择 Codex 原生菜单中真实可见的“新建本地工作树”，使项目配置的 Codex 环境初始化脚本正常执行；Panel 不再自行创建 worktree，而是在 Codex 创建任务后保存实际线程、worktree 和分支。Jira 关联任务会使用 Jira Key 生成原生任务标题、执行提示和分支命名要求，不再使用 Panel Issue 标识。适用的实现与代码审核阶段使用全局配置，默认采用 Codex 执行与官方 Codex CLI 审核，启动或连接类临时失败最多重试两次；服务重启后只有原对话和开发上下文仍一致时才恢复。需要用户输入时显示“等待你的回复”，只有评论或关联对话回复后才重新入队；其他执行失败显示“重新准备执行”，由用户明确重试，自动扫描不会自行重试阻塞工作。Jira 暂停的任务要等 Jira 恢复进行中后才能继续。队列优先处理手动和恢复来源，Jira 授权与周期扫描来源再按 Issue 优先级排序；自动化菜单显示等待数量、运行数与容量、阻塞和失败数量。worktree 会保留到审核，并只在 Issue 完成、目录干净且分支已合并时自动清理。准备阶段先确认运行位置再填入提示词；等待手动发送时不派发新的自动任务。准备失败不会改变手动 Issue 的状态；自动派发失败会明确提示“未确认开始”，不再误称执行已经停止。

同一 Jira、同一仓库的拆分任务串行复用一个原生执行会话、分支和 worktree，每张任务保留独立状态与活动。同仓库依赖可在前置任务进入待审核后继续，跨仓库依赖仍等待完成。会话确认接手后会清除过期队列错误。已有的独立会话不会被自动迁移或合并。

原生会话活动与看板共用实时会话进度；无法读取时显示“状态未知”，不再误报“空闲”。

看板筛选支持名称和优先级排序，显示设置可开启创建日期。普通任务详情可创建子任务，仪表盘按顶层任务等权计算进度并计入子任务完成比例。任务和评论可记录外部 Agent 会话并复制终端恢复命令，详见[任务组织与外部会话](docs/fork-capabilities.md#task-organization-and-external-sessions)。
...
每个项目都有一个全局唯一的项目 Key，由 1-12 位字母或数字组成；新 Issue 使用稳定的 `KEY-N` 编号。创建项目界面要求填写 Key；CLI 和自动仓库注册可省略 `--issue-key`，由服务分配下一个可用 Key。可在“项目文档”后的“项目设置”中原子迁移本地项目 Key 和历史本地 Issue 编号，任务与会话关联保持不变。CLI 只读查询既接受 Panel Issue ID，也接受其关联的 Jira Key。
...
服务运行时，在 Codex Panel App 的“偏好设置 → 工作流 → 配置工作流”进入偏好设置内的工作流页面，侧栏保持可用，页面切换带有简短淡入位移动画并遵循减少动态效果设置；保存或点击“返回偏好设置”即可返回。可以分别为 AI 规划、任务执行、代码审核、任务交接选择有顺序的已安装 Skill，所有本地项目共用。阶段留空时分别使用 Codex 原生 Plan、默认 Codex 执行、官方 Codex CLI 审核、Panel 内置交接。任一选中 Skill 在目标项目不可用时，整个阶段使用默认流程并提示缺失项。进入适用阶段时读取当前配置，包括已有对话的后续阶段；已完成阶段不会重跑。需要沿用原规划流程时，依次选择 `grill-with-docs`、`to-spec`、`to-tickets`；执行阶段可选 `implement`。不会根据本机安装情况自动配置。默认行为与限制见[英文详细说明](docs/fork-capabilities.md#configurable-ai-workflows)。
...
`$handoff-panel --issue PROJECT-123 重点保留验收结论` 可以在任意 Codex 对话中使用。它会先解析目标 Issue 和全局交接配置，使用选中的 Skill 或 Panel 内置总结生成脱敏临时文档，再把文档逐字作为 Codex Agent 的 `AI 对话交接` 评论附加到 Issue。发布失败时仍保留临时文档；不要求安装或修改个人 `handoff` Skill。
...
针对 ChatGPT 桌面版（已核对 26.917.51856 和 26.917.71314 的源码结构），可在启动器 **偏好设置 → 显示与系统** 开启 **移除账号额度不足时 API 发送限制**，首次安装适配时自动重载一次 Codex 界面。之后开关即时生效，无需重启 Panel 服务或刷新 Codex；未发送的输入保持不变。此开关默认关闭，独立于隐藏额度提示；关闭后立即恢复原有额度判断。开启时，Panel 适配本地 Codex 输入框，避免登录账号额度耗尽时错误禁用自定义模型服务的发送入口，保留现有登录和服务凭据。Panel 自动发现当前输入框模块并识别额度与 provider 代码结构，文件哈希或压缩变量名变化不再需要逐版本登记；无法识别的结构保持原行为。兼容范围与验证限制见[自定义服务发送](docs/fork-capabilities.md#custom-provider-send-availability)。
...
无论当前位于会话页，还是 Plugins、Sites 等原生页面，都可以直接点击 Panel 入口打开任务面板。再次打开时会恢复上次选择的任意项目，包括“所有项目”；Panel URL 显式包含 `project` 参数时仍以 URL 为准。Panel 已加入 Codex 原生导航历史：返回回到打开面板前的页面，前进重新打开面板；从面板进入原生目的地后，也能返回面板。显示状态跟随实际路由，不再匹配按钮或命令标题；切换主题等不导航的操作保持面板打开。

已绑定会话的本地任务也可以点击“在新对话打开”，主动打开新输入框并填入任务与交接内容；首次手动发送后才更新绑定。见[英文工作流说明](docs/fork-capabilities.md#configurable-ai-workflows)。

“在对话中打开”会选择对应的原生 Codex 项目，并在本地和 SSH 项目中都打开尚未发送的输入框。本地草稿使用 `$manage-panel`，包含 Issue 编号、标题、最新“AI 对话交接”和 `panelctl` 读取位置，并在执行前刷新 Issue；SSH worker 无法调用本机 `panelctl`，所以远程草稿直接包含当前 Issue 描述、评论和开发上下文。两种草稿都跟随 Panel 界面语言、不显示内部路由标记且不会自动发送；只有第一次真实发送创建 Codex 任务后，Panel 才会写回对应的本地或 SSH thread binding，SSH Issue 也只在此时移到处理中。记录的 ID 可通过 Codex 原生路由桥接点击打开。每个 Issue 可以绑定一个 Git 分支或一个 worktree；选项从所选 Codex 项目的仓库中扫描，而不是手工输入。该集成复用 Codex 现有的项目、输入框和路由标记，导航连接通过 React 祖先属性找到已有 MemoryRouter，并观察 push、replace 和 go；不会替换 `fetch`、加载私有代码块或编辑 Codex 数据文件。

本地内嵌 AI 对话也可以通过聊天标题栏中的关联菜单绑定到 Issue。打开菜单时会直接加载对话原始项目中的活跃 Issue，即使当前看板正在查看另一个项目，也可以正常改绑或取消关联。关联后的对话会显示在 Issue 的活动时间线中，点击即可打开对应的本地聊天。桌面端可以拖动聊天标题栏移动窗口，从上边或左边调整大小，或者最大化和还原；Panel 会在下次打开时恢复普通窗口的位置、尺寸和最大化状态。移动端继续全屏显示，不会覆盖桌面布局记录。Markdown 图片引用本地 PNG、JPEG、GIF 或 WebP 文件时，会通过 Panel 仅限回环访问的图片接口显示；路径包含空格时必须使用 URI 编码或尖括号。发送 `/handoff` 或 `/交接` 可以让同一 Codex 线程总结当前讨论；命令后还可以追加需要重点保留的内容。加入 `--issue ISSUE-ID` 可以把交接记录到指定的活跃 Issue，而不是当前关联的 Issue，例如 `/交接 --issue PROJECT-123 重点保留验收结论`。

需要把交接附加到 Panel Issue 时，使用 `$handoff-panel --issue ISSUE-ID [交接重点]`；原生和内嵌 Codex 对话都适用。它使用全局交接配置或内置总结，逐字复用脱敏后的临时文档；发布失败时保留文档并明确报告部分失败，不修改个人 Skill。成功的交接会进入 Issue 活动流程，供后续 Codex 任务读取。Issue 的状态、优先级、负责人、工作流、开发上下文和重复周期使用跟随 Panel 明暗主题的菜单，不再依赖浏览器原生下拉层。
