...
Jira 与全部关联执行 Issue 都完成后，Panel 会把自己管理的规划和执行对话从活跃对话列表归档。已有 run、消息和 Codex thread ID 继续保存在本地并可读取；已经运行的 turn 会保留到结束后再从活跃列表消失。Jira 重新打开后会创建新的返工或规划对话，不会继续使用已归档历史。
