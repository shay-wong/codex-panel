...
针对 ChatGPT 桌面版 26.917.51856，可在启动器 **偏好设置 → 显示与系统** 开启 **修复自定义 API 发送限制**，再到 **运行概览 → 重启服务** 生效。此开关默认关闭，独立于隐藏额度提示；关闭后重启服务即可恢复原有行为。开启时，Panel 适配本地 Codex 输入框，避免登录账号额度耗尽时错误禁用自定义模型服务的发送入口，保留现有登录和服务凭据。兼容范围与验证限制见[自定义服务发送](docs/fork-capabilities.md#custom-provider-send-availability)。
