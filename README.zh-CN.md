...
看板筛选支持名称和优先级排序，显示设置可开启创建日期。任务详情可创建子任务，仪表盘按顶层任务等权计算进度并计入子任务完成比例。任务和评论可记录外部 Agent 会话并复制终端恢复命令，详见[任务组织与外部会话](docs/fork-capabilities.md#task-organization-and-external-sessions)。
