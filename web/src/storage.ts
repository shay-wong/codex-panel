...
export const PROJECT_BOARD_DISPLAY_SETTINGS_KEY_PREFIX = "panel.project-board-display-settings.v3.";
  ...
  storageWrite = storageWrite.then(async () => {
    ...
    while (true) {
      try {
        ...
        const error = new Error(`Panel storage returned ${response.status}`);
...
export async function initializePanelStorage() {
  try {
...
export const panelStorage: Pick<Storage, "getItem" | "setItem" | "removeItem"> = {
  getItem(key) {
