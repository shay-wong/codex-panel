...
interface PanelGanttTask extends GanttTask {
  panelStatus: Task["status"];
  panelTitle: string;
  panelUnread: boolean;
  panelAssigneeType: Task["assignee"]["type"] | null;
  panelAssigneeName: string;
  panelAssigneeAvatarUrl: string | null;
  panelAssigneeInitial: string;
  panelGroup: boolean;
  panelCount: number;
}
...
export function GanttView({ tasks, presentations, hasActiveFilters, zoom, hideCompleted, todayRequest, onOpenTask, onUpdate }: GanttViewProps) {
  ...
  useEffect(() => {
    ...
    instance.config.columns = [
      {
        ...
        min_width: 190,
        template: (item) => {
          const task = item as PanelGanttTask;
          if (task.panelGroup) {
            return `<div class="gantt-grid-group"><strong>${escapeHtml(task.panelTitle)}</strong><span>${task.panelCount}</span></div>`;
          }
          return `<div class="gantt-grid-issue"><strong>${escapeHtml(task.panelTitle)}</strong>${task.panelUnread ? '<i class="task-unread-dot" aria-label="有未读更新"></i>' : ""}</div>`;
        },
      },
    ];
    ...
    instance.templates.task_class = (_start, _end, item) => {
      const task = item as PanelGanttTask;
      return task.panelGroup ? "gantt-group-task" : `gantt-status-${task.panelStatus}`;
    };
    instance.templates.task_text = (start, end, item) => {
      const task = item as PanelGanttTask;
      if (task.panelGroup) return "";
      ...
      const avatar = task.panelAssigneeType === "agent"
        ? `<img src="/codex-agent-logo.png" alt="">`
        : task.panelAssigneeAvatarUrl
        ? `<img src="${escapeHtml(task.panelAssigneeAvatarUrl)}" alt="">`
        : `<span>${escapeHtml(task.panelAssigneeInitial)}</span>`;
      return `<span class="gantt-bar-content"><i class="gantt-bar-assignee${task.panelAssigneeType === "agent" ? " is-agent" : ""}" title="${escapeHtml(task.panelAssigneeName)}">${avatar}</i><span class="gantt-bar-copy"><strong>${escapeHtml(task.panelTitle)}</strong><small>${dateLabel}</small></span></span>`;
    };
    const rowClass = (item: GanttTask) => {
      const task = item as PanelGanttTask;
      if (task.panelGroup) return `is-group gantt-status-${task.panelStatus}`;
      return `gantt-status-${task.panelStatus}${task.panelUnread ? " is-unread" : ""}`;
    };
    ...
    instance.attachEvent("onAfterTaskUpdate", (id, item) => {
      const ganttTask = item as PanelGanttTask;
      const task = tasksRef.current.find((candidate) => candidate.id === String(id));
      if (!task || ganttTask.panelGroup || item.unscheduled) return true;
  ...
  useEffect(() => {
    ...
    for (const group of GANTT_GROUPS) {
      ...
      data.push({
        ...
        panelAssigneeName: "",
        panelAssigneeAvatarUrl: null,
        panelAssigneeInitial: "",
        panelGroup: true,
        panelCount: groupTasks.length,
      } as PanelGanttTask);
      ...
      for (const task of groupTasks) {
        ...
        data.push({
          ...
          panelUnread: presentations[task.id]?.unread ?? false,
          panelAssigneeType: task.assignee.type,
          panelAssigneeName: task.assignee.name,
          panelAssigneeAvatarUrl: task.assignee.avatarUrl,
          panelAssigneeInitial: Array.from(task.assignee.name.trim())[0] ?? "·",
          panelGroup: false,
          panelCount: 0,
        } as PanelGanttTask);
      }
