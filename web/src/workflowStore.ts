...
export const WORKFLOW_STATE_KEY_PREFIX = "panel.workflow.workspace.";
...
export function readLegacyWorkflowWorkspace(projectId: string): unknown | null {
  try {
    const raw = panelStorage.getItem(workflowStorageKey(projectId));
...
export function clearLegacyWorkflowWorkspace(projectId: string): void {
  panelStorage.removeItem(workflowStorageKey(projectId));
}
