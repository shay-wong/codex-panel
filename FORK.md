...
自定义服务发送（本次修复，等待上游吸收）：ChatGPT 26.917.51856 的本地 Codex composer 在 `m7` 中无条件合并两项官方账号额度限制，导致独立 API provider 也被禁用。启动器“偏好设置 → 显示与系统 → 修复自定义 API 发送限制”独立开关默认关闭，以 `customProviderQuotaFix` 持久化；修改后在“运行概览”重启服务生效。路径：`launcher/src/App.tsx` → `src-tauri/src/main.rs` 的 `set_launcher_preference` → preferences JSON → injector。受管 injector 预读已核对的 `app-primary-aaee46b7f0ce.js`，仅改写该额度表达式并绝对化此构建的字面量导入，保留原始 `import.meta.url`；将同步 importmap 安装代码拼入既有 new-document 注册，用 MutationObserver 在 documentElement 出现时插入精确 URL → blob 映射，再忽略缓存重载。关闭或升级时沿用旧注册撤销与重载，不依赖移除 importmap DOM 节点。实际 Codex Framework 153 中 Fetch 未触发，因此已移除响应拦截实现；Electron 42 的隔离缓存复现不能作为生产根因证明。复用线程 provider 优先、原生模型配置次之的判定，仅对已识别的本地非 `openai` provider 排除额度子条件；必须保留外部禁用、策略错误及后续空输入、上传等原生提交检查，不改登录、凭据、全局额度或官方 App 文件，不识别的源码保持原样。隐藏横幅开关不影响此设置。代码：`scripts/codex-provider-quota.mjs`、`scripts/codex-injector.mjs` 和两处运行时打包清单。验证：`node --test test/codex-provider-quota.test.mjs test/injector.test.mjs`；静态核对全部 87 项依赖导入与 118 处 `import.meta.url`，隔离 Chromium 153 验证模块替换及关闭恢复。日志区分模块准备与实际执行，不记录凭据或消息正文；登录会话发送尚待用户验证。用户入口：中英文 README；详解：`docs/fork-capabilities.md#custom-provider-send-availability`；记录：中英文 CHANGELOG Unreleased。来源定位：`git log -S'rewriteCodexProviderQuota' -- scripts/codex-provider-quota.mjs`。合并时保持 provider 识别和两项额度条件的边界；客户端原生提供等价修复后移除对应构建适配。
