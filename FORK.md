...
- 显式会话绑定不变量：主动创建的 Codex 会话只有在用户明确要求时才可通过 `panelctl conversation bind ISSUE_ID` 绑定 Panel Issue 或 Jira 需求；调用 Skill、提及或读取 Issue、评论 Issue、处于相同仓库均不得隐式绑定。本地已保存会话必须从其 Codex project assignment 与 session metadata 解析完整项目、主机和实际 worktree 身份，不得依赖当前打开 Panel 的其他会话；远端会话仍必须使用宿主提供的完整身份。禁止覆盖其他会话；绑定 Jira 时只建立或恢复本地规划记录，不修改 Jira 字段或状态。
