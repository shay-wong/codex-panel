...
自定义服务发送（本次修复，等待上游吸收）：ChatGPT 26.917.51856 的本地 Codex composer 在 `m7` 中无条件合并两项官方账号额度限制，导致独立 API provider 也被禁用。受管 injector 在既有 renderer reload 前安装 Fetch 响应拦截；重载必须使用 `Page.reload({ignoreCache:true})`，否则 Electron 的模块缓存会跳过拦截，仅为已核对的 `app-primary-aaee46b7f0ce.js` 改写该表达式；复用线程 provider 优先、原生模型配置次之的判定，仅对已识别的本地非 `openai` provider 排除额度子条件。必须保留外部禁用、策略错误及后续空输入、上传等原生提交检查，不改登录、凭据、全局额度或官方 App 文件；不识别的版本/源码保持原样。此适配由启动器“偏好设置 → 显示与系统 → 修复自定义 API 发送限制”独立开关控制，默认关闭，以 `customProviderQuotaFix` 持久化；修改后在“运行概览”重启服务，关闭后同样重启以恢复原始脚本。隐藏横幅开关不影响此设置。开关路径：`launcher/src/App.tsx` → `src-tauri/src/main.rs` 的 `set_launcher_preference` → preferences JSON → injector。代码：`scripts/codex-provider-quota.mjs`、`scripts/codex-injector.mjs` 和两处运行时打包清单。验证：`node --test test/codex-provider-quota.test.mjs test/injector.test.mjs`；隔离浏览器验证真实 Fetch 拦截及发送状态；Electron 42.3.0 / Chromium 148 的隔离 `app://` 复现确认普通重载可能跳过拦截，忽略缓存重载可以应用或撤销适配；这不是生产根因确认，实际安装的 Codex Framework 为 Chromium 153，用户在忽略缓存重载后仍报告发送按钮禁用。诊断日志记录开关读取、Fetch 启用/命中及 app-primary 脚本名称、类型、状态和缓存来源，不记录 URL 参数、凭据或消息正文；真实适配与登录会话发送尚未验证。用户入口：中英文 README；详解：`docs/fork-capabilities.md#custom-provider-send-availability`；记录：中英文 CHANGELOG Unreleased。来源定位：`git log -S'rewriteCodexProviderQuota' -- scripts/codex-provider-quota.mjs`。合并时保持 provider 识别和两项额度条件的边界；客户端原生提供等价修复后移除对应构建适配。
