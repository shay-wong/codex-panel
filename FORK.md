...
- 行为不变量：每个项目保存一个由 1-12 位大写字母或数字组成、全局唯一的 Project Key；新 Issue 使用 `KEY-N`。Project Key 只能通过显式原子迁移修改，迁移同步重编号该项目中仍使用旧 Key 的 `KEY-N` Issue，保持任务 UUID 与关联关系不变，其他前缀的历史 Issue 不变。旧项目升级优先保留首个合法历史前缀；冲突项目按项目创建顺序获得确定性的数字后缀。本地 SQLite、Cloud D1 和一次性 Cloud 数据迁移保持一致。
