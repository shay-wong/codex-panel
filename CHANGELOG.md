...
- Fixed the short-lived generated launcher so it runs as a UI agent and does not leave a separate temporary application in the Dock after handing off to Codex.
...
- Fixed generated-launcher cold starts so the injector waits for Codex's initial `app://` document to finish loading before performing the one CSP-bypass reload required by the Panel iframe, avoiding both the official fallback error page and `ERR_BLOCKED_BY_CSP`.
