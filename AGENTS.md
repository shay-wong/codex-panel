...
- A waiting issue does not stop the rest of the batch. Continue every other eligible lane while one item waits for CI, local review, user input, or an external dependency.
...
- Do not repeat environment discovery in every task. Reuse a verified toolchain, project ID, packaged `panelctl`, and stable coordinator-owned runtime until evidence shows that one changed.
...
- The installed or injected Panel runtime used for issue reads and writes is coordinator-owned. An execution conversation must not stop it, replace its descriptor, overwrite the installed App, or reuse its CDP profile for worktree validation.
...
- Write one initial Panel comment for claim, grouping, and E3 path, and one final comment for implementation, verification, review, PR, CI, and limitations.
...
Use this local review classification:
...
- Review the exact final candidate SHA. If main advances, repeat the local review only when integration changes reviewed functional logic or creates a real overlapping risk; a conflict-free merge commit or trivial targeted fix uses focused local review and direct-path verification.
...
- CI may run as soon as each PR reaches a stable exact head. Run independent required local reviews in parallel after each candidate reaches its final head; do not serialize them through the coordinating conversation.
...
- When a Panel issue is linked to a GitHub Issue, keep completion-state changes synchronized across both systems. Record the local state change in the GitHub Issue and the GitHub state change in the local issue. The release gate below controls when the GitHub Issue can be closed.
