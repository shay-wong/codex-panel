...
On launch, the manager validates its own app signature and bundled runtime, the recorded hashes of Node.js and the official Codex CLI, and the official `ChatGPT.app` designated requirement plus its pinned main-executable path and hash before starting code. It starts the loopback-only Panel service and either attaches to the actual CDP port of an existing Codex or starts the official `/Applications/ChatGPT.app` with CDP before injecting the Panel sidebar. It retires Panel injectors only on the selected port and never replaces its managed injector with a detached process. A normally opened Codex cannot gain CDP after startup; if the manager reports that Codex must restart, quit Codex completely and use **Start Service** again. Stopping or quitting waits for only the injector and Panel service it owns to exit, while the official ChatGPT/Codex application and its CDP endpoint remain running until you quit ChatGPT itself. It never modifies the official app or its `app.asar`.

Re-run `npm run codex:install` after updating the repository, replacing Node.js, or installing a new `ChatGPT.app` version so the pinned application identity and executable hashes are refreshed. `npm run launcher:install` remains available as a compatibility alias. The latest manager log is written to `~/Library/Logs/Codex Panel.log`.
