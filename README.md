...
For ChatGPT desktop 26.917.51856, enable **移除账号额度不足时 API 发送限制** in **偏好设置 → 显示与系统**, the adapter reloads the Codex interface once when first installed. Later switch changes apply immediately without restarting the Panel service or refreshing Codex, preserving the current draft. This independent switch defaults to off; disabling it immediately restores the original quota checks. Panel adapts the local Codex composer so an exhausted ChatGPT account does not disable sending through a custom model provider. The existing login and provider credentials are preserved. See [custom-provider sending](docs/fork-capabilities.md#custom-provider-send-availability) for compatibility and verification limits.
