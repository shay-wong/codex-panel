...
For ChatGPT desktop 26.917.51856, enable **移除账号额度不足时 API 发送限制** in **偏好设置 → 显示与系统**, then use **运行概览 → 重启服务** in Panel. This reloads the Codex interface; quitting and restarting Codex is not required. Reconnection prepares the replacement before the native module loads, then reloads the client without cached scripts. This independent switch defaults to off; disabling it and restarting restores the original behavior. Panel adapts the local Codex composer so an exhausted ChatGPT account does not disable sending through a custom model provider. The existing login and provider credentials are preserved. See [custom-provider sending](docs/fork-capabilities.md#custom-provider-send-availability) for compatibility and verification limits.
