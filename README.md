...
[简体中文](README.zh-CN.md) | [Fork capabilities](docs/fork-capabilities.md) | [Changelog](CHANGELOG.md)
...
The board includes dashboard, list, Gantt, and archived-issue workflows. The horizontal list follows the issue-board column and card hierarchy, while the vertical list remains compact; synced Jira issues show their external key when available. Issues can carry start and due dates and can be moved from their detail view while retaining their linked conversation. A local Jira connection can sync open issues assigned to the signed-in Jira user into a dedicated Jira project. Each Jira requirement can select one or more repository projects and link existing execution issues from those repositories; the searchable, scrollable repository picker includes every local Codex repository and registers an unlisted Panel project only when the selection is saved. An execution issue can belong to at most one Jira requirement, and Jira refreshes do not overwrite its local work. Repository changes in activity use project names rather than internal IDs. The linked Jira card on an execution issue returns to the requirement inside Panel; its detail view retains the separate external Jira action and shows many repositories as the first repository plus `+N`, with the full list in its tooltip. A waiting Jira requirement with saved repository links can create one execution issue and one formal native Codex task per repository, move Jira to in progress, and release the issues to waiting only after every repository is ready; retrying a partial failure continues the same operation without duplicating work. Complex requirements can instead open a formal native Codex planning task that defaults to **Approve selected actions** (`workspace-write`). With no linked repository the task is projectless, with one it opens in that project, and with several Panel first asks which project to use. Panel fills the editable composer with the generated planning prompt and the globally selected planning Skills, or uses native Codex Plan when none are available; the user sends it manually. After approving a native plan, leave Plan mode before saving the Spec or publishing tickets. Formal planning and execution tasks open through Codex's native task route and stay out of the bottom-right embedded chat, which is reserved for temporary questions. User Skills installed as direct symbolic links under `~/.agents/skills` remain real selected Skill references instead of plain `$name` text. If the renderer reloads after sending but before Panel records the binding, the pending intent survives and the next planning click recovers the unique Codex conversation containing the exact Jira key instead of creating a duplicate. The resulting Spec is stored on the Jira requirement, and confirmed repository tickets publish as linked backlog issues with their dependencies. Planning remains available before a repository is selected, and a Codex startup failure after sending shows its actual diagnostic. Jira description lines whose numbered items begin with `#` render as an ordered list without changing ordinary Panel Issue Markdown. Jira or repository changes require review before another publication, pause unstarted issues from entering execution, and cancel only superseded work that has not started; started work remains visible as a planning constraint. Opening the Jira project refreshes it at most once per minute, and users can also sync manually. Panel applies a refresh only after every Jira search page succeeds, keeps cached issues visible when Jira is unavailable, and requires confirmation before switching to a different Jira account. The Jira header and settings show the last attempt, last success, open and unknown counts, and actionable failures. Changes to a synced Jira issue's title, description, priority, labels, due date, or status are written directly back to Jira. The connection supports Basic Auth with an account password or Jira Cloud email and API token, and Bearer Auth with a Jira Data Center or Server personal access token. Credentials are stored in the local Panel data directory, and Jira is unavailable in Cloud mode; use HTTPS unless the Jira server is on a trusted private network.
...
Repository projects use a Panel-owned persistent execution queue instead of Codex Scheduled Tasks. Enable automatic claiming from the project automation menu to scan unlinked waiting issues every 5, 10, 15, 30, or 60 minutes and execute Jira-linked issues after their Jira requirement authorizes work. For manual work, **Prepare execution** opens an unsent Codex draft with **Manage Panel**. After reading the issue, the agent loads configured implementation and review stages only when applicable. Choose **Current directory** (the default) or **New worktree**; existing execution contexts are reused. The issue is linked and moves to in progress only after you send the draft. Automatic claiming still submits automatically; disabling automatic claiming stops new automatic entries without freezing existing work, while pausing the project holds every queued item. The global default permits three concurrent executions per project and can be set from 1 through 8; each project may follow it or choose its own 1-8 limit, with no cross-project total cap. New automatic execution selects the visible native **New local worktree** menu item, so the project's Codex environment setup runs; Panel then stores the actual thread, worktree, and branch returned by Codex instead of creating its own worktree. Jira-linked execution uses the Jira key in the native task title, prompt, and branch naming instruction rather than the Panel issue identifier. Applicable implementation and code-review stages use the global selections, defaulting to Codex execution and the official Codex CLI reviewer, and retries transient startup or connection failures twice. Execution that needs input shows **Waiting for your reply** and resumes only after a comment or completed user turn; other execution failures show **Prepare again** for an explicit retry. Automatic scans ignore blocked work, Jira-paused work stays disabled, and service restarts resume only when the original conversation and development context still match. Manual and resumed work run first; Jira-authorized and scanned work are then ordered by issue priority. The automation menu shows waiting, running/capacity, blocked, and failed totals. Worktrees remain through review and are removed after completion only when clean and merged.
...
Each project has one globally unique Project Key containing 1-12 letters or numbers. New Issues use it as their stable `KEY-N` prefix. The create-project UI requires the Key; CLI and automatic repository registration may omit `--issue-key` to allocate the next available Key. **Project Settings**, immediately after **Project Docs**, can migrate a local project's Key and historical local Issue identifiers atomically while keeping task and conversation links intact. Read-only CLI lookups accept either the Panel Issue ID or its linked Jira key.
...
Task entry attaches only **Manage Panel**. It reads the issue first, then fetches applicable stages with `panelctl workflow get <stage> --project PROJECT_ID --json`. Pure research checks sources and conclusions without loading implementation or code-review Skills, even when saving a report. Authorized code changes use the configured execution and review stages; an explicit code-review request can use review directly. `in_review` also means a research report awaits your confirmation.

Each stage has a complete editable workflow template alongside its Skill selection. Customize the steps and output format, or restore the default. `{{skill_instructions}}` inserts the selected Skill order or the stage's default method. Custom templates replace the default steps; task context and fixed Panel rules are attached separately and explained in the editor.

In the Codex Panel App, open **Preferences → Workflow → Configure workflows** while the Panel service is running. The editor opens as a page inside Preferences, with the sidebar available and a brief page transition that respects reduced motion. Save or use **Back to preferences** to return. Planning, execution, code review, and task handoff each accept an ordered selection of installed Skills, shared by all local projects. Leave a stage empty to use native Codex Plan, default Codex execution, the official Codex CLI reviewer, or Panel's built-in handoff. If any selected Skill is unavailable in the target project, the entire stage uses its default and reports the missing selection. The current configuration is read when an applicable stage begins, including later stages in an existing conversation; already completed stages are not rerun. To retain the previous planning flow, select `grill-with-docs`, `to-spec`, and `to-tickets` in that order; `implement` is an optional execution choice. Nothing is selected automatically from your installed Skills. See [workflow defaults and limits](docs/fork-capabilities.md#configurable-ai-workflows).
...
`$handoff-panel --issue PROJECT-123 preserve the acceptance decisions` works from any Codex conversation. It resolves the target Issue and global handoff selection, then creates a redacted temporary document using the selected Skills or Panel's built-in summary. It attaches that document verbatim as an `AI 对话交接` Codex Agent comment; publication failure keeps the document available. No personal `handoff` Skill is required or modified.
...
For ChatGPT desktop (verified source layouts: 26.917.51856 and 26.917.71314), enable **移除账号额度不足时 API 发送限制** in **偏好设置 → 显示与系统**, the adapter reloads the Codex interface once when first installed. Later switch changes apply immediately without restarting the Panel service or refreshing Codex, preserving the current draft. This independent switch defaults to off; disabling it immediately restores the original quota checks. Panel adapts the local Codex composer so an exhausted ChatGPT account does not disable sending through a custom model provider. Panel discovers the current composer module and matches its quota/provider code structure, so asset hashes and minified variable renames do not require a version entry. Unrecognized structures retain native behavior. The existing login and provider credentials are preserved. See [custom-provider sending](docs/fork-capabilities.md#custom-provider-send-availability) for compatibility and verification limits.
...
The management window uses React and Radix Themes in Tauri's WebView. Runtime overview and grouped preferences share the same neutral controls and light/dark theme as the embedded workflow editor; service, process, and file operations retain the existing Rust commands. A fixed left sidebar separates Runtime overview, Preferences, and About; Runtime overview places Open Panel with the status summary and service actions. Version information, update checks, installation, and release notes live in About; logs and runtime diagnostics remain in Runtime overview. The main area uses a compact status summary, labeled service actions, and three status rows; preferences use grouped form rows with a prominent global-workflow entry. Codex connection and actual embedded-Panel visibility are reported separately; queued opens wait for renderer readiness instead of being reported as failures, and native Codex actions wait briefly for the first host-bridge heartbeat when Panel has just appeared. Action buttons keep loading visible for at least 300 ms, then briefly retain a visible success or failure state; service start, stop, and restart keep the window responsive while process lifecycle work completes; and the browser entry validates and preserves the launcher's private loopback URL. Launch preferences, update and Release actions, log and data controls, and expandable runtime details follow below. Its header and the macOS app/Dock icon use matching light and dark Codex icons with the `PANEL` ribbon and follow the system appearance.

Fork versions are independent of upstream, starting at `0.0.1-fork` with tag `v0.0.1-fork`. Upstream merges do not change the Fork version. Existing local `0.1.0` development builds require a one-time manual installation to switch to this version line.

The app checks the Fork's GitHub Releases automatically at most once every 24 hours and caches successful results for that period; temporary failures are retried after 5 minutes, while anonymous API rate limits are cached until GitHub's reset time. Manual checks always bypass the cache. It first uses a locally installed, authenticated `gh` CLI and falls back to the anonymous GitHub API when unavailable. Rate limits, network failures, no releases, current versions, and available updates are reported separately. Only normalized `vX.Y.Z-fork` tags are update candidates; available updates are downloaded and signature-verified in the app, then installed only after user confirmation. On macOS, the confirmation uses a native foreground dialog; other platforms retain the Tauri dialog. Builds without the fork verification key explain why installation is unavailable. See [update packaging](docs/fork-capabilities.md#signed-in-app-updates).
...
For local tasks, **Open in new conversation** also works with an existing binding. It opens a new unsent draft with task and handoff context; the first manual send updates the binding. See [conversation workflow](docs/fork-capabilities.md#configurable-ai-workflows).

“Open in conversation” selects the corresponding native Codex project and opens an unsent composer in both local and SSH projects. Local drafts use `$manage-panel`, include the issue identifier, title, latest “AI conversation handoff,” and `panelctl` read location, and refresh the issue before acting. SSH drafts include the current issue description, comments, and development context because the remote worker cannot call the local `panelctl`. Both drafts follow the Panel interface language, omit internal routing markers, and remain unsent; only the first real send creates the Codex task and writes the new local or SSH thread binding back to the issue, with SSH issues moving to in progress at that point. Recorded IDs are clickable through Codex's native route bridge. Each issue can bind either one Git branch or one worktree; the options are scanned from the selected Codex project's repository instead of being typed by hand. The integration uses Codex's existing project, composer, and route markers; navigation locates the existing MemoryRouter through React ancestor props and observes its push, replace, and go methods. It does not replace `fetch`, load private chunks, or edit Codex data files.

Local embedded AI conversations can also be linked to an issue from the link menu in the chat header. When opened, the menu loads active issues directly from the conversation's original project, even if another project is currently visible, and can switch or remove the link while Codex is idle. Linked conversations appear in the issue activity stream and open the exact local chat when selected. On desktop, drag the chat header to move the window, resize it from the top or left edges, or maximize and restore it; Panel remembers the normal position, size, and maximized state the next time it opens. Mobile keeps the chat full-screen without replacing the saved desktop layout. Markdown images that reference local PNG, JPEG, GIF, or WebP files render through Panel's loopback-only image endpoint; paths containing spaces must use URI encoding or angle brackets. Send `/handoff` or `/交接` to have the same Codex thread summarize the discussion; optional text after the command tells it what to emphasize. Add `--issue ISSUE-ID` to record the handoff on a specific active issue instead of the linked issue, for example `/交接 --issue PROJECT-123 preserve the acceptance decisions`.

Use `$handoff-panel --issue ISSUE-ID [handoff focus]` to attach a handoff from a native or embedded Codex conversation. It uses the global handoff selection or built-in summary, preserves the redacted temporary document verbatim, and reports publication failure without removing that document. Personal Skills remain unchanged. Successful handoffs are recorded in the activity stream for the next Codex task. Issue status, priority, assignee, workflow, development context, and recurrence use themed menus that follow the Panel's light or dark appearance instead of browser-native dropdowns.
...
For a reverse tunnel that connects to the local listener, set `CODEX_PANEL_TRUSTED_ORIGINS` to the tunnel's public HTTPS origin, for example `https://board.example.test`. Multiple origins are comma-separated. The variable cannot be empty, and duplicate origins (including normalized forms such as a trailing slash or default HTTPS port) are rejected at startup. Entries must otherwise be exact HTTPS origins; paths, queries, fragments, credentials, and wildcards are rejected. A reverse proxy or tunnel must preserve a loopback socket connection and either preserve the exact configured public `Host` or rewrite it to a local/private host. It must preserve any `Origin` supplied by the browser and add the configured public HTTPS origin only when the header is absent, including for `GET` and `HEAD`; forwarded headers are not used for this decision. Requests using a configured public Host or Origin can use ordinary Panel HTTP and realtime endpoints, but device-local capability routes remain unavailable even though the tunnel socket is loopback. Requests from direct local or private-LAN origins keep their existing behavior. The legacy `CODEX_TASKBOARD_TRUSTED_ORIGINS` name remains a fallback.
