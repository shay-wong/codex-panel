...
After Jira and every linked execution issue are done, Panel archives its managed planning and execution conversations from the active conversation list. Existing runs, messages, and Codex thread IDs remain stored and readable; a turn already running stays visible until it finishes. Reopened Jira work creates new rework or planning conversations instead of continuing archived history.
