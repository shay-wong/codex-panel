...
const SESSION_COOKIE_NAME = "__Host-panel_session";
...
function unauthorized() {
  return json(
    ...
    { "www-authenticate": 'Basic realm="Codex Panel", charset="UTF-8"' },
  );
}
...
async function authenticate(request, env) {
  ...
  if (credentials) {
    ...
    sessionCookie = await createSessionCookie(username, sharedSecret);
  } else {
    username = await decodeSessionUsername(request, sharedSecret);
...
function parseAttachmentHeaders(request) {
  ...
  const kind = request.headers.get("x-panel-attachment-kind")
    ?? request.headers.get("x-taskboard-attachment-kind");
