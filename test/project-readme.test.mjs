...
async function startServer() {
  const directory = await mkdtemp(path.join(os.tmpdir(), "codex-panel-readme-test-"));
  const app = createPanelServer({ dataDirectory: directory });
...
test("Database: getProjectReadme returns empty record for fresh project", async () => {
  ...
  let db;
  try {
    db = new PanelDatabase(path.join(directory, "panel.sqlite"));
...
test("CLI: panelctl project readme get and set", async () => {
