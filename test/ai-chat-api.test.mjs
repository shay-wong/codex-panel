...
const panelBase = "http://127.0.0.1:43210/instance-token/";
globalThis.document = { baseURI: panelBase };
...
test("local AI API client follows the fixed catalog, thread, turn and interrupt contract", async () => {
  ...
  try {
    ...
    assert.equal(
      calls.at(-1).path,
      `${panelBase}api/local/ai/catalog?projectId=project%20%2F%20one`,
    );
...
test("thread event subscription listens only for public snapshot hints and closes cleanly", () => {
  ...
  try {
    ...
    assert.equal(openedUrl, `${panelBase}api/local/ai/threads/thread%20%2F%201/events`);
