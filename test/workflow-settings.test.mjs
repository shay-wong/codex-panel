...
test("global workflow settings drive default, custom and unavailable-Skill actions", async () => {
  ...
  try {
    ...
    assert.match(planned.composerText, /Clarify requirements first\./);
    assert.doesNotMatch(planned.composerText, /\{\{skill_instructions\}\}/);
    ...
    assert.ok(remoteInput.some((item) => item.type === "text" && item.text.includes(custom.prompts.handoff)));
