...
test("issue detail renders descriptions and comments with GFM markdown", () => {
  ...
  assert.match(
    detailSource,
    /\{descriptionDocument\s*\?\s*<DescriptionDocument\s*value=\{descriptionDocument\}\s*referenceTasks=\{referenceTasks\}\s*onOpenTask=\{onOpenTask\}\s*attachments=\{descriptionAttachments\}\s*enableImagePreview\s*onOpenAttachment=\{handleAttachmentDownload\}\s*onCopyAttachmentPath=\{onCopy\}\s*\/>\s*:\s*text\("添加描述…", "Add description…"\)\}/,
  );
  assert.match(
    detailSource,
    /commentBody && \(\s*<div className="comment-body">\s*<DescriptionDocument\s*value=\{commentBody\}\s*referenceTasks=\{referenceTasks\}\s*onOpenTask=\{onOpenTask\}\s*attachments=\{comment\.attachments\}\s*enableImagePreview\s*onOpenAttachment=\{handleAttachmentDownload\}\s*onCopyAttachmentPath=\{onCopy\}\s*\/>\s*<\/div>\s*\)/s,
  );
